...
def draw_all_results(img_dict,s_obj_dict,m_obj_dict,deveice_id_list,img_height,img_width,obj_range):
   
    device_count = len(deveice_id_list)
    # Create canvas
    canvas_img_height = img_height + obj_range
    ...
    for elem in deveice_id_list:
        ...
        if s_obj_dict.__contains__(elem) and s_obj_dict[elem] is not None:
            ...
            canvas_img[img_height:canvas_img_height,img_count*(img_width+obj_range):img_count*(img_width+obj_range)+img_width] = resize_s_obj_img
    ...
    for elem in obj_camera_array.trackers_dict:
        for obj in obj_camera_array.trackers_dict[elem].objects_pool:
            if obj_camera_array.trackers_dict[elem].objects_pool[obj].update_status is True:
                ...
                obj_vehicle = Vehicle(id=obj_camera_array.trackers_dict[elem].objects_pool[obj].id,color=obj_camera_array.trackers_dict[elem].objects_pool[obj].color,scale=(obj_canvas.scale_dict[elem][0],obj_canvas.scale_dict[elem][1]))
                canvas_img = obj_vehicle.draw(canvas_img,count*obj_canvas.cam_region_width+disp_center_y,disp_center_x)
        count+=1
                # cv2.circle(canvas_img,(disp_center_y,disp_center_x),2,(255,255,255),2)   
    # x,y = 23,4
...
class Canvas(object):
    ...
    def get_scale(self):
        device_count = len(self.obj_camera_array.trackers_dict)
        self.cam_region_width = int(self.img_width/device_count)
