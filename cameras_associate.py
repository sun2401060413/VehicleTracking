"""
Camera associate
written by sun zhu on 2019-03-21, version 1.0
Updated by sun zhu on Sep 29, 2020, version 1.1.0
"""
...
# ==== Default files path setting, you could comment the code line below and set your own path in new project ====
from Common import cam_names, data_path, box_info, roi_info, save_path, track_info, associate_info
...
def get_associate_dict(filepath):
...
class SingleCameraSTP(object):
    def __init__(   self,
        ...
        self.var_beta_x = var_beta_x         # x range multiplier
        self.var_beta_y = var_beta_y            # y range multiplier
        
        self.get_motion_params()
        self.nx_predictor = GaussDistribution(
                                    self.time_interval*self.motion_params_4_all['mean_x'],
                                    self.time_interval*self.var_beta_x*self.motion_params_4_all['var_x'])
        self.ny_predictor = GaussDistribution(
    ...
    def update_predictor(self, tracking_record=None, time_interval=None, var_bata_x=1, var_bate_y=1):
    ...
    def get_probability_map(self, base_x, base_y, start_x=0, start_y=0, length_x=10, length_y=10, height=100, width=100):
        return get_probability_map(self.nx_predictor, self.ny_predictor, base_x, base_y, start_x, start_y, length_x, length_y, height, width)
        
    def get_probability(self, x, y, base_x, base_y):
        """Calculate the probability of pos
            x, y: pos to be calculated;
            base_x, base_y: predicted pos
        """
        delta_x = x - base_x 
        delta_y = y - base_y
...
class MultiCamerasSTP(object):
    ...
    def get_probability_map(self, base_x, base_y, t_interval=None, height=300, width=80):
        nx_predictor = GaussDistribution(  t_interval*self.obj_single_camera_stp_cam_1.motion_params_4_all['mean_x'],
                                            t_interval*self.var_beta_x*self.obj_single_camera_stp_cam_1.motion_params_4_all['var_x'])
        ny_predictor = GaussDistribution(  t_interval*self.obj_single_camera_stp_cam_1.motion_params_4_all['mean_y'],
...
def get_STP_in_multi_cameras(obj_single_camera_stp_cam_1, obj_single_camera_stp_cam_2, associate_dict):
    ...
    objs_associate_info = []
    for k in associate_dict:    # mapping dict
        if isinstance(k, (str)):
            if k in pt_trace_1 and k in pt_trace_2:
                for i in range(min(len(pt_trace_1[k]), len(pt_trace_2[k]))):    # 考虑路径长短不同，取两路径最短的，要不会有控制
                    objs_associate_info.append([k, pt_trace_1[k][i], pt_trace_2[k][i]])
        else:
            for i in range(min(len(pt_trace_1[str(k)]), len(pt_trace_2[str(k)]))):
...
def get_probability_map(obj_nx, obj_ny, base_x, base_y, start_x=0, start_y=0, length_x=10, length_y=10, height=100, width=100):

    x = np.linspace(start_x, start_x+length_x, width)
    y = np.linspace(start_y, start_y+length_y, height)
...
def gaussian_test():
    ...
    obj_2D = GaussDistribution(mu, Sigma)
...
def match_based_on_spatial_temperal_prior_test_1(tracker_record, pt_obj, t_interval=30):
    ...
    test_id = '0'
    # file path
    img_root = data_path[device_id]
    save_root = r"E:\Project\CV\trajectory\VehicleTracking\results\sct\001\test"

    # SingleCameraSTP
    obj_STP = SingleCameraSTP(tracker_record, pt_obj, t_interval, var_beta_x=1, var_beta_y=1)
    # obj_STP.update_predictor(tracker_record, 30, var_bata_x=2, var_bate_y=4)
       
    # test on id 1
    for v, elem in enumerate(obj_STP.perspective_trace[test_id]):
        if v+t_interval > len(obj_STP.perspective_trace[test_id])-1:
            break
        fname_1 = '{:0>4d}.jpg'.format(elem[1])
        fname_2 = '{:0>4d}.jpg'.format(elem[1]+t_interval)
        

        test_x, test_y = obj_STP.perspective_trace[test_id][v+t_interval][0][0], obj_STP.perspective_trace[test_id][v+t_interval][0][1]
        print("PrePos:", elem[0][0], ";", elem[0][1], ";", test_x, ";", test_y)
        ...
        p_map = obj_STP.get_probability_map(base_x=elem[0][0], base_y=elem[0][1], start_x=0, start_y=0, length_x=15, length_y=110, height=110, width=15)
...
def trace_display_test(tracker_record, obj_id='0', img_path=None):
    """Display object trajectory on chosen image"""
...
if __name__ == "__main__":
    ...
    # device_id = 0
    #
    # img_filepath_1 = data_path[device_id]
    # img_savepath_1 = save_path[device_id]
    # tk_json_filepath_1 = track_info[device_id]
    # pt_savepath_1 = roi_info[device_id]
    #
    # # cam B
    # associate_device_id = associate_cam[device_id]
    #
    # img_filepath_2 = data_path[associate_device_id]
    # img_savepath_2 = save_path[associate_device_id]
    # tk_json_filepath_2 = track_info[associate_device_id]
    # pt_savepath_2 = roi_info[associate_device_id]
    #
    # from data_generator import load_tracking_info
    # tracker_record_1 = load_tracking_info(tk_json_filepath_1)
    # tracker_record_2 = load_tracking_info(tk_json_filepath_2)
    #
    ...
    #
    # # # ======= TEST: trace display ======
    # # trace_display_test(tracker_record_1, obj_id='0', img_path=os.path.join(data_path[0], "0001.jpg"))
    #
    ...
    # #                                                 5)
    #
    # associate_dict_c1_c2 = get_associate_dict(associate_info)[cam_names[1]]
